FusionCharts.ready(function () {
    ...
    btn.addEventListener('click', function () {
        var date = new Date(month.value + currYear);
        // Get the day of the year
        var startIndex = getDayOfYear(date);
        // Get the month's index
        var monthIndex = date.getMonth() + 1;
        // Get the number of days in that month
        var endIndex = new Date('2015', monthIndex, 0).getDate();
        endIndex += startIndex;

        /**
     * @description
     * The zoomTo() method zooms the zoom line chart to view a specific range of data.

     * @param {number} startIndex: Index of the data point starting at which the chart will be zoomed into
     * @param {number} format: Index of the data point until which the chart will be zoomed into
     * usage: chartObj.zoomTo(startIndex,endIndex);
     *         
     */

        visitChart.zoomTo(startIndex - 1, endIndex);
    });
});